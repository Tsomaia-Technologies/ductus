---
name: engineer
description: Execute an RFC 0001 implementation task. Use when the user wants to implement a specific task from the agentic layer redesign plan, says "implement task N", or wants to start the next task.
...
1. Ask the user which task number to execute (0-14), or check `rfc/tasks/README.md` for the next unstarted task.
2. Read the task brief from `rfc/tasks/` (e.g., `rfc/tasks/03-conversation-class.md`).
3. Verify dependencies are met by checking if the files created by prerequisite tasks exist. If dependencies are not met, inform the user and stop.
