...
export class IntentProcessor {
  ...
  async process(params: {
    ...
    while (true) {
      ...
      const { value: event, done } = await this.consumeNext(
        eventsOut,
        nextValue,
        sourceSubscriber,
      )
