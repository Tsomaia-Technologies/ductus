...
**No, Revision 03 does not fully capture the "closing of the loop."**

While it added the *Planner* (Start), it left the *Feature Reviewer* (End) implicit in the "per_feature" checks. In your requested workflow, the **Feature Reviewer** is a critical agent that must load the `SPEC_GENERATED` event (the "Contract") and validate the final product against it before the system can exit.

Here is **Revision 04**.
I have updated **Section 8.2** to visualize the *entire* food chain (User → Planner → Task Maker → Engineer ↔ Reviewer → Feature Verifier) and added **Section 8.3** to explicitly define the **Verification Phase**.
...
## 8. The "Food Chain" Workflow (Planning & Verification)

To prevent ambiguous tasks from reaching the implementation phase, Ductus v2 implements a strict top-down, bottom-up validation flow.

### 8.1 The Planner Agent

The Planner is a specialized agent that sits "above" the food chain. Its goal is **Interrogation**, not implementation.

1. **Input:** User's raw request + Optional existing plan.
2. **Process:** The Planner analyzes the request for gaps, edge cases, and missing context.
3. **Interaction:** It emits `PLANNER_QUESTION` (Volatile) events to the User via `InputProcessor`.
4. **Output:** Once satisfied, it emits `SPEC_GENERATED` (Durable), which contains a rigorous Technical Specification.
...
The configuration now supports **Scoped Contexts** via key-value objects. Scopes can be automatically resolved via file matching or explicitly requested by the Planner/User.
...
interface ScopeConfig {
  ...
  roles: Record<string, {
    ...
    strategies: Array<{
      id: string;
      model: string; // e.g., 'claude-3-opus' for Planning, 'gpt-4o' for Coding
      template: string;
    }>;
  }>;
}
...
### 9.1 Scope Resolution Logic

1. **Explicit:** If the Task/Plan specifies `scope: "frontend"`, the Orchestrator enforces that scope's rules.
2. **Automatic:** If `scope` is undefined, the Orchestrator matches the modified files (via `git diff`) against the `match` globs in the config.
