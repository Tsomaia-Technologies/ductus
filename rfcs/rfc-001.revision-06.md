...
The system is composed of 10 specialized, single-purpose state machines (Processors) connected via a central concurrent **CryptographicMultiplexer Hub (The Circuit)**. There are no direct function calls between processors; they communicate strictly via events.
...
To prevent Ledger bloat from high-frequency streams (e.g., progress bars, LLM tokens) and ensure the Type Machine only processes "Atomic Facts," the `PersistenceProcessor` implements a **Volatility Filter**.
...
sequenceDiagram
    ...
    participant OP as Development Processor
    participant TP as Tool Processor
    participant SM as Type Machine
    
    AP->>Hub: Yield AGENT_REPORT_RECEIVED (files: ["a.ts"])
...
4. **Agent Atomicity:** If an agent stream is interrupted (network failure/kill), partial content is **discarded**. An `AGENT_INTERRUPTED` event is emitted to trigger retry logic, preventing the storage of corrupted JSON/Type.
...
4. **Tombstone Injection:** A synthetic `HALLUCINATION_DETECTED` event is injected in place of the quarantined history (e.g., "Previous attempt failed due to file path hallucinations").
...
### 8.3 The Full Type Lifecycle (Mermaid)
...
* **Boundary:** The `DevelopmentProcessor` performs the interpolation. The `ToolProcessor` receives only the raw, fully-resolved string and is oblivious to the context substitution.
