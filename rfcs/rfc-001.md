# Request for Comments: Ductus v2 Architecture (Event-Sourced CryptographicMultiplexer)
...
Ductus v2 is a complete foundational rewrite. It adopts **Event Sourcing**, a pure **Type Machine (XState)**, and a **Reactive Stream Processor (CryptographicMultiplexer)** architecture. It introduces a rigorous Zero-Trust Orchestration pipeline, Proof-of-Work cryptographic event ledgers, and decoupled Agent topologies.
...
```mermaid
graph TD
    Boot[Bootstrapper] -->|Hydrate| SM[Type Machine]
    Boot -->|Wire| Hub[Event Hub/CryptographicMultiplexer]
    
    subgraph "The Circuit (Runtime)"
        Hub <--> SMNode[Type Machine Processor]
...
## 4. The "Brain": Type Machine (XState)
...
This is the most critical logic layer for reliability. It stands between the Agent and the Type Machine.
...
sequenceDiagram
    ...
    Hub->>S: Type Machine transitions to next phase
```
...
4.  **Pump:** Events are pumped into the Type Machine to restore the logic tree.
