...
Implement the `QualityProcessor`. Once the `DevelopmentProcessor` has successfully merged and tested all individual micro-tasks in a feature, the `QualityProcessor` acts as the final gatekeeper (The Feature Reviewer). It operates at the macro-level, running holistic suite tests and invoking the `AuditorRole` agent to compare the entire finalized Codebase Type directly against the original `SPEC.md`.
